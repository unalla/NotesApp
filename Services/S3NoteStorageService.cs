...
namespace CloudNotes.Services
{
    public class S3NoteStorageService : INoteStorageService
    {
        ...
        public async Task<Note> GetNote(string username, Guid? noteId)
        {
            ...
            if (string.IsNullOrWhiteSpace(username))  username = "unalla";//throw new ArgumentNullException("username");
            if (noteId == null) throw new ArgumentNullException("noteId");

            string noteObjectKey = GetNoteObjectKey(username, noteId.Value);
